    ...
    <div className="space-y-7">
      ...
      <section className="space-y-6">
        <div className="rounded-3xl border border-white/10 bg-white/6 p-6 backdrop-blur-xl shadow-[0_12px_50px_rgba(2,6,23,0.35)]">
          ...
          {data?.allSourcesFailed && (
            <div className="mt-4 rounded-2xl border border-rose-300/20 bg-rose-400/10 px-3 py-2 text-sm text-rose-200">
              当前全部源失败：已切换 fallback 数据，请检查网络或数据源可用性。
            </div>
          )}
        </div>
                  ...
                  <div key={item.id} className="rounded-3xl border border-white/10 bg-white/5 p-4">
                    <div className="flex flex-wrap items-center gap-2">
                      <span
                        ...
                        title={item.publishedAt ? `publishedAt: ${item.publishedAt}` : `publishedAt 缺失，使用 fetchedAt: ${item.fetchedAt}`}
                      >
                        {item.displayTime}
                      </span>
          ...
          <div className="xl:col-span-5 space-y-6">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              ...
              <div className="mt-2 space-y-1 text-xs text-slate-300">
                <div>当前快讯流数据来源：{data?.dataMode === "real" ? "真实" : "fallback"}</div>
                <div>当前是否命中缓存：{data?.cached ? "是" : "否"}</div>
                <div>当前是否有源失败：{data?.hasSourceFailure ? "是" : "否"}</div>
                {(data?.sourceStatus || []).map((item) => (
                  <div key={item.source}>
                    {item.source}：{item.status} / {item.mode} / cached:{item.cached ? "yes" : "no"}（{item.message}）
                  </div>
                ))}
              </div>
            </div>
